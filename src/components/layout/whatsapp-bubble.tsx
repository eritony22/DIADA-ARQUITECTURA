...
export default function WhatsappBubble({ number }: { number?: string }) {
  ...
  return (
    <motion.a
      ...
      whileTap={{ scale: 0.96 }}
      className="fixed bottom-6 right-6 z-[80] flex h-14 w-14 items-center justify-center rounded-full bg-ink text-bone shadow-[0_10px_30px_rgba(0,0,0,0.35)]"
    >
      <span className="absolute inset-0 -z-10 animate-ping rounded-full bg-ink/50" />
